...
outdir <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/primetime_data_to_remake_figures/MainFig6_dualincubation_outputs"
indir <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/primetime_data_to_remake_figures/MainFig6_dualincubation"

outpdf <- file.path(outdir, paste0("coords_dbl_assignment_knn_impute_with_UMAP.", Sys.Date(), ".pdf"))
outrda <- file.path(outdir, paste0("coords_dbl_assignment_cleaned.rda"))
# outrdsimpute <- file.path(outdir, paste0("coords_dbl_assignment_knn_impute.", Sys.Date(), ".rds"))
...
inf.meta <- file.path(indir, "scchix_outputs.H3K4me1-H3K9me3.2021-02-11.setseed.RData")
load(inf.meta, v=T)
...
wmax <- c("0.99"); names(wmax) <- wmax

fits.out.lst <- lapply(wmax, function(w){
  inf <- file.path(indir, "unmix_scchix_inputs_clstr_by_celltype_H3K4me1xH3K9me3.removeNA_FALSE.RData")
  load(inf, v=T)
...
inf.lda <- file.path(indir, "ClusterAnnot.lda_and_datmerged.k4_k9_50kb_genomewide.H3K4me1xH3K9me3.2021-01-31.RData")
load(inf.lda, v=T)
...
rename.repress <- list("Eryths" = "Erythroid", "Bcells" = "Lymphoid", "Granulocytes" = "Myeloid", "HSPCs" = "HSPCs")

dat.umap.long.annot.genomewide.clean <- dat.umap.long.annot.genomewide %>% dplyr::select(-louv.act, -louv.repress) %>%
  rowwise() %>%
  dplyr::mutate(louv.repress.impute = rename.repress[[louv.repress.impute]])
dat.umap.long.annot.genomewide.clean$louv.repress.impute <- factor(x = dat.umap.long.annot.genomewide.clean$louv.repress.impute, levels = c("Erythroid", "Lymphoid", "Myeloid", "HSPCs"))
...
save(dat.umap.long.annot.genomewide.clean, file = outrda)
# saveRDS(coords.dbl.impute, file = outrdsimpute)
